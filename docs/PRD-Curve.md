...
- **정렬**: 추천순(상대적 우선순위 기반), 최신순, 회사명순(한글→영문)
...
### 4.1 주요 엔티티
- **profiles**: 사용자 프로필 (user_id, name) - Supabase RLS 적용
- **JobPosting**: 채용공고 (로컬 스토어)
  - minimumRequirementsCheck: AI 최소 조건 검토 결과
  - companyCriteriaScores: 회사 평가 점수 (세부 설명 포함)
  - keyCompetencies: 핵심 역량 및 AI 평가
- **Experience**: 경력/프로젝트 (로컬 스토어, 기간 정보 포함)
- **TailoredResume**: 공고별 맞춤 이력서 (로컬 스토어)
- **CareerGoal**: 이직 목표 (로컬 스토어, 복수 목표 지원)
...
| 함수명 | 설명 |
|--------|------|
| analyze-job | 공고 URL 분석 및 정보 추출 |
| parse-resume | 이력서 PDF 파싱 (텍스트/OCR) |
| generate-resume | 공고별 맞춤 이력서 생성 |
